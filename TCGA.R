...
readData = getFirehoseData (dataset="LUAD", runDate="20151101",forceDownload = TRUE,
                            Clinic=TRUE, Mutation=FALSE, Methylation=FALSE, RNAseq2_Gene_Norm=FALSE)
...
write.table(mRNA, file = 'TCGA_mixture_LUSC.txt', append = FALSE, 
            quote = FALSE, sep = '\t',
            row.names = TRUE,
            col.names = TRUE 
)
...
results_LUAD_col = ConsensusClusterPlus(ES,maxK=6,reps=5000,pItem=0.8,pFeature=1,
...
results_LUAD_row = ConsensusClusterPlus(t(ES),maxK=6,reps=5000,pItem=0.8,pFeature=1,
...
rownames(ES)[1:10]
ann <- data.frame(case_class = factor(results_LUAD_row[[6]]$consensusClass),
                  TN = c(rep(NA, dim(ES)[1])))
...
ah<- aheatmap(ES, 
              ...
              Rowv = results_LUAD_row[[6]]$consensusTree
...
rownames(ES)[1:10]
ann <- data.frame(case_class = factor(results_LUSC_row[[6]]$consensusClass),
                  TN = c(rep(NA, dim(ES)[1])))
ann$TN[grep('TCGA.........11*', rownames(ES))] <- 'Normal'
ann$TN[is.na(ann$TN)] <- 'Tumor'
